# -*- coding: utf-8 -*-
"""
Created on Wed Apr  1 12:55:22 2020
for AI course:s3w3 car recognization
@author: lwang
"""
#%% Import libraries
import argparse
...
def MY_yolo_filter_boxes(box_confidence, boxes, box_class_probs, threshold = .6):
    # it is the same with below yolo_filter_boxes, but implement in a 'stupid' way
    ...
    # temp = tf.reshape(box_scores, [19,19, -1]) # [19,19,5*80]
    box_classes = K.argmax(box_scores, axis=-1) # [19,19,5]
    box_class_scores = K.max(box_scores, axis=-1) # [19,19,5]
    box_classes2= tf.reshape(box_classes, [-1])
    box_class_scores2 = tf.reshape(box_class_scores, [-1])
    ### END CODE HERE ###
    
    # Step 3: Create a filtering mask based on "box_class_scores" by using "threshold". The mask should have the
    # same dimension as box_class_scores, and be True for the boxes you want to keep (with probability >= threshold)
    ### START CODE HERE ### (≈ 1 line)
    filtering_mask = (box_class_scores2>=threshold) # [1, 19*19*5]
    filtering_mask2 = tf.squeeze(filtering_mask) 
    ### END CODE HERE ###
    
    # Step 4: Apply the mask to box_class_scores, boxes and box_classes
    ### START CODE HERE ### (≈ 3 lines)
    boxes2 = tf.reshape(boxes, [-1, 4])# [19*19*5, 4]
    scores = tf.boolean_mask(box_class_scores2, filtering_mask2)
    boxes_out = tf.boolean_mask(boxes2, filtering_mask2)
    classes = tf.boolean_mask(box_classes2, filtering_mask2)
    ### END CODE HERE ###
   
    return scores, boxes_out, classes
...
def yolo_filter_boxes(box_confidence, boxes, box_class_probs, threshold = .6):
    ...
    # same dimension as box_class_scores, and be True for the boxes you want to keep (with probability >= threshold)
    ### START CODE HERE ### (≈ 1 line)
    filtering_mask = (box_class_scores>=0.5) # [1, 19*19*5]
    print('filtering_mask', filtering_mask.shape)
    
    # Step 4: Apply the mask to box_class_scores, boxes and box_classes
    scores = tf.boolean_mask(box_class_scores, filtering_mask)
    print('scores',scores.shape)
    boxes_out = tf.boolean_mask(boxes, filtering_mask)
    print('boxes',boxes.shape)
    print('boxes_out',boxes_out.shape)
...
def yolo_non_max_suppression(scores, boxes, classes, max_boxes = 10, iou_threshold = 0.5):
    ...
    # print('nms_indices', nms_indices.shape)
    ### END CODE HERE ###
...
#     print("boxes[2] = " + str(boxes[2].eval())) # OR: s2=K.eval(boxes)
...
# folderin = 'images'
# folderout = 'out'
# out_scores, out_boxes, out_classes, _ = predict(sess, folderin, "0003.jpg",
#                                                 folderout, yolo_out)
...
# read image size OR: image_shape = (720., 1280.) 
...
for path in paths:
    ...
    print(image_C)
    out_scores, out_boxes, out_classes, _ = predict(sess, folderin, image_C,
                                                folderout, yolo_out)
